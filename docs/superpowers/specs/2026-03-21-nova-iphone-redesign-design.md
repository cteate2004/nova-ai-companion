...
- Random push notifications 1-3x per day (configurable)
...
- Scheduler picks random times within waking hours (configurable)
...
- Haptic feedback via `navigator.vibrate()` on mic tap, send, task completion
